	...
	unsigned int opt_bitmask)
{
	...
	if (host_vir_addr == NULL) {
		GDL_LOGW("bus_id = %d, addr = 0x%p/0x%08x/0x%08x, NULL!",
	...
	if (no_read_back && (!full_print)) {
		GDL_LOGI("bus_id = %d, addr = 0x%08x, w_val = 0x%08x",
			bus_id, host_addr, val);
	} else if (no_read_back && (full_print && !print_vir_addr)) {
		GDL_LOGI("bus_id = %d, addr = 0x%08x/0x%08x, w_val = 0x%08x",
			bus_id, host_addr, bus_addr, val);
	} else if (no_read_back && (full_print && print_vir_addr)) {
		GDL_LOGI("bus_id = %d, addr = 0x%p/0x%08x/0x%08x, w_val = 0x%08x",
			bus_id, host_vir_addr, host_addr, bus_addr, val);
	} else if (!no_read_back && (!full_print)) {
		GDL_LOGI("bus_id = %d, addr = 0x%08x, w_val = 0x%08x, r_back = 0x%08x",
			bus_id, host_addr, val, read_back_val);
	} else if (!no_read_back && (full_print && !print_vir_addr)) {
		GDL_LOGI("bus_id = %d, addr = 0x%08x/0x%08x, w_val = 0x%08x, r_back = 0x%08x",
			bus_id, host_addr, bus_addr, val, read_back_val);
	} else {
		/* if (!no_read_back && (full_print && print_vir_addr)) */
		GDL_LOGI("bus_id = %d, addr = 0x%p/0x%08x/0x%08x, w_val = 0x%08x, r_back = 0x%08x",
	...
	unsigned int opt_bitmask)
{
	 ...
	 */
	if (!full_print) {
		GDL_LOGI("bus_id = %d, addr = 0x%08x, r_val = 0x%08x",
			bus_id, host_addr, val);
	} else if (full_print && !print_vir_addr) {
		GDL_LOGI("bus_id = %d, addr = 0x%08x/0x%08x, r_val = 0x%08x",
			bus_id, host_addr, bus_addr, val);
	} else {
		/* if (full_print && print_vir_addr) */
		GDL_LOGI("bus_id = %d, addr = 0x%p/0x%08x/0x%08x, r_val = 0x%08x",
			bus_id, host_vir_addr, host_addr, bus_addr, val);
	}
	return val;
}
