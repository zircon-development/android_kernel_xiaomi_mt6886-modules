...
void gps_mcudl_hal_ccif_show_status(void)
{
	...
	GDL_LOGW("busy=0x%x, start=0x%x, rch=0x%x",
	...
	for (id = 0; id < GMDL_CCIF_MISC_NUM; id++) {
		...
		GDL_LOGW("msic_id=%d, mask=0x%x, dummy=0x%x, shadow=0x%x",
	...
	for (ch = 0; ch < GPS_MCUDL_CCIF_CH_NUM; ch++) {
		if (rch_mask & (1UL << ch)) {
			...
			if (ch == GPS_MCUDL_CCIF_CH1) {
				...
				/* WHOLE_CHIP reset */
#if GPS_DL_HAS_CONNINFRA_DRV
				conninfra_trigger_whole_chip_rst(CONNDRV_TYPE_GPS, "FW trigger");
#endif
			} else if (ch == GPS_MCUDL_CCIF_CH4)
